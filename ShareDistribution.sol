    ...
    PausableUpgradeable
{
    ...
    /// @notice Sets the amount of shares for multiple addresses at once
    /// @param participants An array of addresses of the participants
    /// @param participantShares The amount of shares owned by each participant
    function setParticipantSharesBulk(
        address[] memory participants,
        uint256[] memory participantShares
    ) public onlyRole(UPDATER_ROLE) {
        require(
            participants.length == participantShares.length,
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            ...
            uint256 shares = participantShares[i];
